...
const FolderDetail = () => {
  ...
  const handleUploadPdf = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:5000/api/pdfs/${folderId}/upload`,
        formData,
        {
          headers: {
  ...
  const toggleCompletion = async (pdfId, status) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `http://localhost:5000/api/pdfs/${folderId}/${pdfId}/progress`,
        { completed: status },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto text-gray-800">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative">
          <button
            ...
            className="relative p-2 bg-purple-100 rounded-full hover:bg-purple-200"
          >
      ...
      <div
        {...getRootProps()}
        className="border-2 border-dashed rounded-lg p-6 text-center cursor-pointer mb-4 bg-purple-50 hover:bg-purple-100"
      >
        <input {...getInputProps()} />
